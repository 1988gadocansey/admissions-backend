...
def vocationalSSCE(request):
    if request.method == 'GET':
        ...
        print("download vocation");
        year = request.POST.get('year')
        if year != "":

            data = Resultuploadmodel.objects.filter(form__yearofadmission=year).filter(
                form__admitted=True).order_by('examtype').values(
                'form__applicationnumber',
                'form__lastname',
                'form__firstname',
                'form__gender',
                'form__phone',
                'form__formerschoolnewid__name',
                'form__ formerschool',
                'form__admissiontype',
                'form__programmeadmittedid__name',
                'indexno',
                'year',
                'month',
                'examtype',
            ...
            response[
                'Content-Disposition'] = "attachment; filename=" + year + "_admitted_applicants" + ".xlsx"
            ...
            ws['A2'] = "ADMISSIONS"
            ws['A3'] = str(year) + " ADMISSIONS CATEGORIES "
            ...
            columns = [
                ...
                (u"GENDER", 60,),
                (u"PHONE", 30),
                (u"INSTITUTION ATTENDED", 30),
                (u"ADMISSION TYPE", 30),
                (u"PROGRAMME ADMITTED", 30),
            ...
            for obj in data:
                ...
                if year in ["2018/2019", "2017/2018"]:
                    ...
                    if "MAY/JUNE" in obj['month'] or "may/june" in obj['month'] or "june" in obj['month'] or "may" in \
                            obj[
                        ...
                        'month'] or "MAY/" in obj['month'] or "JUNE/" in obj['month'] or "JUNE" in obj[
                        'month'] or "MAY" in \
                            obj[
                                'month']:
                        monthExam = 6
                    ...
                    # admissionType = obj['admissiontype']
                    # if not yearExam:
                #
                ...
                if year == '2021/2022':
                    dataSet = [
                        obj['form__applicationnumber'],
                        str(obj['form__lastname']).upper(),
                        str(obj['form__firstname']).upper(),
                        str(obj['form__gender']).upper(),
                        obj['form__phone'],

                        str(obj['form__formerschoolnewid__name']).upper(),

                        str(obj['form__admissiontype']).upper(),
                        str(obj['form__programmeadmittedid__name']).upper(),
                        obj['indexno'],
                        yearExam,
                        monthExam,
                        examType
                    ...
                    dataSet = [
                        ...
                        str(obj['form__ formerschool']).upper(),
                        str(obj['form__admissiontype']).upper(),
                        str(obj['form__programmeadmittedid__name']).upper(),
                        obj['indexno'],
                        yearExam,
            ...
            export_xlsx.short_description = u"Admission Categories XLSX"
            return response
...
def totalAdmittedByYear(year, program):
    ...
    data = Applicantmodel.objects.filter(yearofadmission=year).filter(
        programmeadmittedid=program).count()
...
def sales(request):
    ...
    records = Sale.objects.filter(free=0).filter(deleted=0).filter(
...
def export_xlsx(request):
    ...
    if year != "" and programme != "":
        ...
        data = Resultuploadmodel.objects.filter(form__yearofadmission=year).filter(form__admitted=True).filter(
            form__programmeadmittedid=programme).order_by('form__lastname').values('indexno',
                                                                                   'form__lastname',
                                                                                   'form__applicationnumber',
                                                                                   'form__firstname',
                                                                                   'year',
        ...
        for obj in data:
            ...
            dataSet = [
                str(obj['form__lastname'] + ' ' + obj['form__firstname']).upper(),
                obj['form__applicationnumber'],
                obj['indexno'],
                yearExam,
                monthExam
...
def refire_sms(request, object_id):
    ...
    regular = "Hi " + applicant.firstname + "you have been admitted to TTU. Visit " \
                                    "admissions.ttuportal.com to print your admission letter." + str(
        formDetails)


    technical = "Hi " + applicant.firstname + "you have been admitted to TTU. Visit " \
                                      "admissions.ttuportal.com to print your admission letter." + str(
        formDetails)

    mature = "Hi " + applicant.firstname + "you have been admitted to TTU. Visit " \
                                   "admissions.ttuportal.com to print your admission letter." + \
             str(
                 formDetails)
